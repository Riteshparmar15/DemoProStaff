...
export default function IntroSplash({ onComplete }) {
  ...
  useEffect(() => {
    // Every load / refresh starts at the top of the page
        ...
        >
          <div className="absolute right-4 top-4 z-10 md:right-8 md:top-6">
            <button
              type="button"
              onClick={finish}
              className="rounded-full border border-cream/25 bg-cream/5 px-4 py-2 text-xs font-semibold tracking-[0.12em] text-cream/80 uppercase transition hover:border-cream/50 hover:text-cream"
            >
              Skip
            </button>
          </div>
            ...
            <div className="mx-auto grid max-w-5xl grid-cols-5 gap-2 md:gap-4">
              {journeySteps.map((step, i) => (
                ...
                >
                  ...
                  <p className="text-[10px] font-semibold tracking-[0.12em] text-cream/65 md:text-xs">
                    {step.id} {step.label}
                  </p>
                </motion.div>
              ))}
